...
def filtrado(event):
    ...
    cargar_tabla1(tecla_presionada,
                  "SELECT id,NumHabitacion,Tipo,Costo,Dias,SubTotal,Descuento,Total FROM cliente WHERE Estado = 1 AND NumHabitacion LIKE")
...
def guardar():
    ...
    if cajaHabitacion.get() != '' and item_selected2(True) != 0 and cajaDias.get() != '' and radio_var.get() != 0:
        ...
        mycursor.execute("INSERT INTO cliente (NumHabitacion,Tipo,Costo,Dias,SubTotal,Descuento,Total,Estado) VALUES (%s,%s,%s,%s,%s,%s,%s,%s)",
...
def borrar():
    ...
    if seleccionado:
        mycursor.execute("DELETE FROM cliente WHERE id=%s ", [seleccionado[0]])
        mydb.commit()
        for i in tree.get_children():
            tree.delete(i)
        cargar_tabla1(
...
cajaHabitacion = customtkinter.CTkEntry(master=root_tk, width=120, height=25)
