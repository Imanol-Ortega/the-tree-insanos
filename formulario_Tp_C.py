...
def retornar_Menu(formulario_Tp_C, menu):
    formulario_Tp_C.withdraw()
    menu.deiconify()


def ventana_Formulario_TP(menu):
    ...
    ventana.protocol("WM_DELETE_WINDOW", ventana)
    ...
    def seleccioncombo(event):
        ...
        for i in tabla.get_children():
            tabla.delete(i)
        mycursor.execute("SELECT NuHabitacion, Tipo, SUM(Dias),SUM(Total) FROM cliente WHERE Estado=2 AND Tipo LIKE %s ", [
                         "%"+seleccionado+"%"])
    ...
    tabla.grid(row=9, column=1, columnspan=2, padx=10,
