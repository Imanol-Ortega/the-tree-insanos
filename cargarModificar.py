...
def ventana_Carga_Modificacion(menu):
    def ventana_Error(a):
        ...
        if (a==1):
            ...
            mensaje_error = ttk.Label(ventana_error, text="Ya existe el tipo de habitación")
        if(a==2):
            ventana_error.title("Error")
            mensaje_error = ttk.Label(ventana_error, text="No se encontró el tipo de habitación")
        if(a==3):
            ventana_error.title("Exito")
            mensaje_error = ttk.Label(ventana_error, text="La modificación se ha realizado con éxito")
    ...
    titulo = ttk.Label(frm, text="Descripción").grid(column=1, row=3, columnspan=2)
    caja_Descripcion = ttk.Entry(frm)
    caja_Descripcion.grid( column=1,row=4, padx=20, pady=20, sticky="nsew", columnspan=2)

    #?  Costo y su respectiva caja de texto
    ttk.Label(frm, text="Costo").grid(column=1, row=5, columnspan=2)
    caja_Costo = ttk.Entry(frm)
    caja_Costo.grid( column=1,row=6, padx=20, pady=20, columnspan=2, sticky="nsew")

    #?  Botones para guardar
    ttk.Button(frm, text="Guardar", command=guardar_Habitacion).grid(column=1, row=7, columnspan=2, padx=20, pady=20, sticky="nsew")
    #?  Y modificar
    ttk.Button(frm, text="Modificar", command=modificar_Habitacion).grid(column=1, row=8, columnspan=2, padx=20, pady=20, sticky="nsew")
    #?  salir/retroceder
    ttk.Button(frm, text="Atras", command=lambda: retornar_Menu(cargarModificar, menu)).grid(column=1, row=9, padx=20, pady=20, sticky="nsew", columnspan=2)
    ...
    tabla.grid(row=10,column=1, columnspan=2,padx=10, rowspan=8,pady=10, sticky="nsew")
